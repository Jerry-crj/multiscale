...
set sock1 [socket localhost 8800]; 
fconfigure $sock1 -buffering none;

set toUpdate 0;
set totalSubtituteSteps 20;
set count 0;
set dt 0.01;
set steps 0;
...
while {$currentTime < 8.8} { 
	...
	if {$toUpdate == 1} {
        ...
        } elseif {$currentTime > 7.0} {
         
            if {$count < $totalSubtituteSteps} {
                incr  count
            }
            set lambda [expr 1.0-(1.0*$count)/$totalSubtituteSteps]
